...
import { useLatencyLog, getCurrentPageUrl, type LatencySource, type LatencyLogEntry } from './useLatencyLog';
 ...
 */
function classifySource(url: string): LatencySource {
 ...
 * ```
 */
export function useLatencyTrackedFetch(config: LatencyTrackerConfig = {}) {
  ...
  const trackedFetch = useCallback(
    async (
      ...
      init?: LatencyTrackedFetchOptions
    ): Promise<Response> => {
      const {
        skipLogging,
        forceLog,
...
export function createLatencyTrackedFetch(options: {
  ...
  return async function latencyTrackedFetch(
    input: RequestInfo | URL,
    init?: LatencyTrackedFetchOptions
  ): Promise<Response> {
    const {
