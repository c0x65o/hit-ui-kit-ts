...
export function Autocomplete({
  ...
  const listboxId = useMemo(
    () => `hit-autocomplete-${Math.random().toString(36).slice(2, 8)}`,
    []
  );
  ...
  return (
    <div ref={containerRef} style={styles({ position: 'relative' })}>
      {label && (
        <label
          style={styles({
            display: 'block',
      ...
      <div style={styles({ position: 'relative' })}>
        ...
        {loading && (
          ...
          >
            <Loader2 size={16} color={colors.text.muted} className="animate-spin" />
          </div>
        )}

        {clearable && (selected || inputValue) && (
          <button
            type="button"
      ...
      {open && (
        ...
        >
          {loading ? (
            <div style={styles({ padding: `${spacing.sm} ${spacing.md}`, color: colors.text.muted })}>
              {searchingMessage}
            </div>
          ) : options.length === 0 ? (
            <div style={styles({ padding: `${spacing.sm} ${spacing.md}`, color: colors.text.muted })}>
              ...
              return (
                <div
                  ...
                  style={styles({
                    ...
                    borderBottom:
                      idx < options.length - 1 ? `1px solid ${colors.border.default}` : 'none',
                  })}
                >
                  <div style={styles({ fontWeight: 500 })}>{opt.label}</div>
                  {opt.description && (
                    <div style={styles({ marginTop: 2, fontSize: ts.bodySmall.fontSize, color: colors.text.muted })}>
                      {opt.description}
                    </div>
                  )}
                </div>
