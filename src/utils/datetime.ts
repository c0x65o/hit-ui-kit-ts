...
type HitConfigLike = {
  reporting?: { timezone?: string | null } | null;
} | null | undefined;

function getHitConfig(): HitConfigLike {
  if (typeof window === 'undefined') return undefined;
  const w = window as unknown as { __HIT_CONFIG?: unknown };
  return (w.__HIT_CONFIG as any) as HitConfigLike;
}

export function getReportingTimezone(): string | undefined {
...
function isLikelyIsoDateTimeString(v: string): boolean {
...
function isLikelyDateOnlyString(v: string): boolean {
  // YYYY-MM-DD
  return /^\d{4}-\d{2}-\d{2}$/.test(v);
}

function parseDateOnlyToSafeDate(value: string): Date | null {
...
function toDate(value: unknown): { date: Date; kind: 'datetime' | 'date' } | null {
...
export function isDateTimeLikeColumnKey(columnKey: string): boolean {
...
export function formatDateTimeValue(
  ...
  if (tz) formatOptions.timeZone = tz;
 ...
 */
export function formatDateTimeCellIfApplicable(columnKey: string, value: unknown): string | null {
  if (!isDateTimeLikeColumnKey(columnKey)) return null;

  if (typeof value === 'string') {
    const s = value.trim();
    if (!isLikelyIsoDateTimeString(s) && !isLikelyDateOnlyString(s)) return null;
