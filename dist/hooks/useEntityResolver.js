...
export function useEntityResolver() {
     ...
     */
    const getLabel = useCallback((entityType, id) => {
        if (!id)
            return null;
     ...
     */
    const isCached = useCallback((entityType, id) => {
        if (!id)
            return true; // Empty IDs don't need resolution
        return Boolean(globalCache[entityType]?.[id]);
    }, []);
    /**
     * Resolve a batch of entity IDs
     */
    const resolveEntities = useCallback(async (requests) => {
        if (!requests.length)
            return;
        ...
        try {
            ...
            await Promise.all(toFetch.map(async (req) => {
                const def = getEntityDefinition(req.entityType);
                if (!def)
                    return;
                ...
                const fetchPromise = (async () => {
                    ...
                    if (req.ids.length <= 5) {
                        ...
                        await Promise.all(req.ids.map(async (id) => {
                            try {
     ...
     */
    const populateFromRowData = useCallback((entityType, idField, labelField, rows) => {
     ...
     */
    const clearCache = useCallback((entityType) => {
        if (entityType) {
...
//# sourceMappingURL=useEntityResolver.js.map