 ...
 * ```
 */
export function useTableFilters(tableId) {
    const [optionsCache, setOptionsCache] = useState({});
    // Track which autocomplete filters should be rendered as dropdowns (small option count)
    const [autocompleteAsDropdown, setAutocompleteAsDropdown] = useState(new Set());
    ...
    useEffect(() => {
        ...
        const selectFilters = filterDefs.filter((f) => (f.filterType === 'select' || f.filterType === 'multiselect') && f.optionsEndpoint);
        // Also pre-fetch autocomplete filters to check if they should be dropdowns
        const autocompleteFilters = filterDefs.filter((f) => f.filterType === 'autocomplete' && f.searchEndpoint);
        if (!selectFilters.length && !autocompleteFilters.length)
            return;
        let cancelled = false;
        setLoading(true);
        const fetchOptions = async (f, endpoint, isAutocomplete) => {
            try {
    ...
    const filters = useMemo(() => {
        return filterDefs.map((def) => {
            ...
            if (effectiveFilterType === 'autocomplete' && def.searchEndpoint) {
                ...
                base.onSearch = async (query, limit) => {
                    try {
...
//# sourceMappingURL=useTableFilters.js.map