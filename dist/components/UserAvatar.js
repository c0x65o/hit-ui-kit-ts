 ...
 * ```
 */
export function UserAvatar({ email, src, name, size = 'md', className }) {
    ...
    return (_jsx(Avatar, { src: imageUrl || undefined, name: displayName, size: size, className: className }));
}
/**
 * Clear the profile picture cache (useful after profile updates)
 */
export function clearUserAvatarCache(email) {
    if (email) {
 ...
 */
export function setUserAvatarCache(email, profilePictureUrl) {
