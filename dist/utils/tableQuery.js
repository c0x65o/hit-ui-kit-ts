...
export function globalFilterValuesToServerFilters(args) {
    ...
    for (const [columnKey, raw] of Object.entries(values || {})) {
        ...
        if (t === 'daterange' && isNonEmptyString(raw)) {
            ...
            const to = (toStr || '').trim();
            if (from)
                out.push({ field: columnKey, operator: 'dateAfter', value: from });
            if (to)
                out.push({ field: columnKey, operator: 'dateBefore', value: to });
            continue;
        }
        ...
        if (t === 'boolean') {
            ...
            if (s === 'true')
                out.push({ field: columnKey, operator: 'isTrue', value: '' });
            else if (s === 'false')
                out.push({ field: columnKey, operator: 'isFalse', value: '' });
            continue;
        }
        if (t === 'multiselect') {
            const arr = toStringArray(raw);
            if (arr.length)
                out.push({ field: columnKey, operator: 'in', value: arr });
            continue;
        }
        if (t === 'select' || t === 'autocomplete') {
            const s = Array.isArray(raw) ? String(raw[0] ?? '').trim() : String(raw).trim();
            if (s)
                out.push({ field: columnKey, operator: 'equals', value: s });
            continue;
        }
        if (t === 'date') {
            const s = Array.isArray(raw) ? String(raw[0] ?? '').trim() : String(raw).trim();
            if (s)
                out.push({ field: columnKey, operator: 'dateEquals', value: s });
            continue;
        }
        if (t === 'number') {
            const s = Array.isArray(raw) ? String(raw[0] ?? '').trim() : String(raw).trim();
